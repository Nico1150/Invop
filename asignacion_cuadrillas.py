...
def agregar_variables(prob, instancia):
    ...
    for i in range(1, instancia.cantidad_trabajadores + 1):
        for j in range(1, 7):
            nombres.append("T" + str(i) + str(j))
    largo = nombres.len()            
    prob.variables.add(obj= [0]*largo, lb = [0]*largo, ub = [1]*largo, types = ['B']*largo, names = nombres)

    #Variables que determinan si cierta orden fue realizada
    ordenes = instancia.ordenes
    for i in range(instancia.cantidad_ordenes):
        prob.variables.add(obj = ordenes[i][1], lb = [0], ub = [1], types = ['B'], names = ["O" + str(ordenes[i][0])])

    #Variables que determinan cuanto hay que pagarle a cada trabajador
    for i in range(1, instancia.cantidad_trabajadores + 1):
        prob.variables.add(obj = [-1000, -200, -200, -100], lb = [0]*4, ub = [30]*4, types = ['B']*4, names = ["W" + str(i) + "1", "W" + str(i) + "2", "W" + str(i) + "3", "W" + str(i) + "4"])    
